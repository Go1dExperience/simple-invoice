...
import { IsIn, IsInt, IsOptional, IsString, Max, Min, IsDateString } from 'class-validator';
...
export class ListInvoicesQuery {
  ...
  @IsOptional() @IsIn(['invoiceDate', 'dueDate', 'totalAmount']) sortBy: 'invoiceDate' | 'dueDate' | 'totalAmount' = 'invoiceDate';
  @ApiPropertyOptional({ enum: ['ASC', 'DESC'] }) @IsOptional() @IsIn(['ASC', 'DESC']) ordering: 'ASC' | 'DESC' = 'DESC';
  @ApiPropertyOptional({ enum: ['Draft', 'Pending', 'Paid', 'Overdue'] })
  @IsOptional() @IsIn(['Draft', 'Pending', 'Paid', 'Overdue']) status?: 'Draft' | 'Pending' | 'Paid' | 'Overdue';
