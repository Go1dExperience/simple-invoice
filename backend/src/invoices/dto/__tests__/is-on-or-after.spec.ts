...
const make = (invoiceDate: string, dueDate: string) => Object.assign(new Dto(), { invoiceDate, dueDate });
...
describe('IsOnOrAfter', () => {
  ...
  it('fails when dueDate is before invoiceDate', async () => {
    ...
    expect(errors[0].constraints?.isOnOrAfter).toBe('dueDate must be on or after invoiceDate');
  });
});
