...
import {
  IsEmail, IsInt, IsNotEmpty, IsNumber, IsOptional, IsPositive, IsString, Min, ValidateNested, IsDateString,
...
export class CreateInvoiceDto {
  ...
  @ApiProperty({ example: '2026-07-03' }) @IsDateString()
  @IsOnOrAfter('invoiceDate', { message: 'dueDate must be on or after invoiceDate' }) dueDate!: string;
  @ApiProperty({ example: 'AUD' }) @IsString() @IsNotEmpty() currency!: string;
  @ApiPropertyOptional() @IsOptional() @IsString() description?: string;
  @ApiProperty({ type: ItemDto }) @ValidateNested() @Type(() => ItemDto) item!: ItemDto;
  @ApiPropertyOptional({ default: 10 }) @IsOptional() @IsNumber() @Min(0) taxPercent: number = 10;
  @ApiPropertyOptional({ default: 0 }) @IsOptional() @IsNumber() @Min(0) discount: number = 0;
}
