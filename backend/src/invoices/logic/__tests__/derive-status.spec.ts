...
describe("deriveStatus", () => {
  it("returns Overdue when an unpaid invoice is past its due date", () => {
    expect(deriveStatus("Pending", new Date("2026-07-01"), today)).toBe(
      "Overdue",
    );
    expect(deriveStatus("Draft", new Date("2026-07-01"), today)).toBe(
      "Overdue",
    );
  });
  ...
  it("returns the persisted status when not past due", () => {
    expect(deriveStatus("Pending", new Date("2026-08-01"), today)).toBe(
      "Pending",
    );
