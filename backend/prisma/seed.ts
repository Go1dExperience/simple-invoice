...
async function main() {
  ...
  for (let i = 1; i <= 32; i++) {
    ...
    const paid =
      status === "Paid" ? total : status === "Pending" ? total / 2 : 0;
    const currency = CURR[i % CURR.length];
    const c = await prisma.customer.create({
      data: { fullname: NAMES[i % NAMES.length], email: `c${i}@example.com` },
    });
    await prisma.invoice.create({
      data: {
        invoiceNumber: `IV-${1000 + i}`,
        invoiceDate: daysFromNow(-40 + i),
        dueDate: daysFromNow(-40 + i + 30),
        currency,
        currencySymbol: SYMBOLS[currency],
        status,
        invoiceSubTotal: money(sub),
        totalTax: money(tax),
        totalDiscount: money(discount),
        totalAmount: money(total),
        totalPaid: money(paid),
        balanceAmount: money(total - paid),
        createdBy: user.id,
        customerId: c.id,
        items: {
          create: { name: `Item ${i}`, quantity: qty, rate: money(rate) },
        },
      },
    });
  }
