...
const HEAD_CLASS = "h-auto p-3 text-xs font-normal uppercase tracking-wider text-slate-500";
...
const Row = ({ invoice, onOpen }: { invoice: InvoiceListItem; onOpen: (id: string) => void }) => (
  <TableRow onClick={() => onOpen(invoice.invoiceId)} className="cursor-pointer border-b border-slate-100 hover:bg-accent/10">
    <TableCell className="p-3.5 tabular-nums">{invoice.invoiceNumber}</TableCell>
    ...
    <TableCell className="p-3.5 text-right font-bold tabular-nums">{invoice.totalAmount}</TableCell>
